...
export class MemStorage implements IStorage {
  ...
  private initializeData() {
    ...
    // Express stories are no longer needed - all stories now come from Firebase
    // Keep this empty to avoid duplication with Firebase stories
    const defaultStories: any[] = [
      // All stories now come from Firebase - no Express stories needed
...
Ve Saflık ile Sevgi, hep onun yanında oldu.`,
    ...
    // No stories to add - Firebase handles all story management now
  }
  ...
  async createStory(insertStory: InsertStory): Promise<Story> {
    ...
    const story: Story = { 
      ...
      views: 0,
      likes: 0,
      tags: (insertStory.tags && Array.isArray(insertStory.tags)) ? insertStory.tags as string[] : [],
